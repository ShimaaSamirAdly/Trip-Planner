...
public class NoteService extends Service implements View.OnClickListener, NoteListener, OnTipListener {
...
//    private String noteString;
    ...
    private ArrayList<NoteModel> notArr = new ArrayList<>();
...
// Command(intent, flags, startId);
       db =new DataBaseAdapter(NoteService.this);

       Log.i("extra", ""+intent.getExtras());
        db.selectTripById(this,intent.getExtras().getInt(ReminderActivity.SERVICE_NOTE));

//        notArr= AppUtil.convertJsonToListOfNoteObject(trip.getNotesJasonString());
//Log.i("noteArrSize",""+notArr.size());
////      Log.i("LogLogggg", noteString);
...
//        noteString= intent.getStringExtra(ReminderActivity.SERVICE_NOTE);
//        Log.i("LogLogggg", noteString);
//    }
...
//        implementClickListeners();
    ...
    private void addFloatingWidgetView(LayoutInflater inflater) {
        ...
        mFloatingWidgetView = inflater.inflate(R.layout.note_floating_widget, null);
...
//                        DataBaseAdapter db =new DataBaseAdapter(NoteService.this);
                        Log.i("Done===", "Done");
//                        db.updateTrip(trip);
...
//                                onFloatingWidgetClick();
    ...
    private void implementClickListeners() {
        ...
        recyclerView= mFloatingWidgetView.findViewById(R.id.my_recycler_view);

        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        //condition hena ????????
        if(notArr !=null) {
            noteAdapter = new NoteAdapter(notArr, this);
...
//        mFloatingWidgetView.findViewById(R.id.close_floating_view).setOnClickListener(this);
    ...
    @Override
    public void onItemChange(NoteModel homeModel,int position) {
        notArr.remove(position);
        notArr.add(position,homeModel);
     notArr.get(position).setNoteStatus(homeModel.getNoteStatus());
     trip.setNotesJasonString(AppUtil.convertNotesListToJsonString(notArr));
//        noteAdapter.notifyDataSetChanged();
        Log.i("change--- ","change is done ");
